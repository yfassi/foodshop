...
export function QrPosterDownload({ slug, restaurantName }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    generateQrDataUrl(buildCustomerOrderUrl(slug), { width: 1024 })
      .then((url) => {
  ...
  const handleDownload = async (format: QrPosterFormat) => {
    ...
    try {
      ...
      await downloadQrPoster({
        format,
        qrDataUrl,
        url: buildCustomerOrderUrl(slug),
        filename: `taapr-affiche-${slug}-${format.toLowerCase()}.pdf`,
        restaurantName,
      });
    } catch (err) {
      console.error(err);
