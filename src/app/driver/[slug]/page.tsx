  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/driver/deliveries?restaurant_slug=${encodeURIComponent(slug)}`
      );
      if (res.status === 401) {
      ...
      <div className="space-y-5 px-4 py-4">
        ...
        <Section title="En cours">
          {mine.length === 0 ? (
            ...
            mine.map((o) => (
              <Link
                key={o.id}
                href={`/driver/${slug}/order/${o.id}`}
                className="block"
              >
                <DeliveryCard order={o} slug={slug} />
              </Link>
            ))
          )}
        </Section>
        ...
        {done.length > 0 && (
          <Section title="Terminées">
            {done.map((o) => (
              <DeliveryCard key={o.id} order={o} slug={slug} dim />
            ))}
          </Section>
        )}
      </div>
