...
export default function DriverOrderDetailPage() {
  const params = useParams<{ publicId: string; orderId: string }>();
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/driver/deliveries?restaurant_public_id=${encodeURIComponent(params.publicId)}`
      );
      if (res.status === 401) {
    ...
    setLoading(false);
  }, [params.publicId, params.orderId, router]);
  ...
  if (!order) {
    return (
      <div className="px-4 py-8 text-center text-sm text-muted-foreground">
        ...
        <Link href={`/driver/${params.publicId}`} className="text-primary">
          Retour
        </Link>
      </div>
    );
